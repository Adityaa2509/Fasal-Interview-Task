...
function SearchOption({setMoviesAdded,setlist}) {
  ...
  const handleAddToList = async (movieId) => {
    try {
     ...
     const resp =  await axios.put(`http://localhost:8080/api/v1/list/add/${listId}`, { movieId },{withCredentials:true});
      // Optionally show a success message or perform other actions
      console.log(resp.data)
      setlist(resp.data)
      setMoviesAdded(prevState => ({
        ...prevState,
        movies: [...prevState.movies, movieId]
      }));
